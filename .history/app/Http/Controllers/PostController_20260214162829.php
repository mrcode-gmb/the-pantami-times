...
class PostController extends Controller
{
    ...
    public function show($post)
    {
        ...
        $trendingPosts = Post::select(['id', 'uuid', 'title', 'slug', 'image', 'credit', 'video_url', 'published_at', 'views', 'category_id'])
            ->where('status', 'published')
            ->whereNotNull('published_at')
            ->where('id', '!=', $post->id)
        ...
        $relatedPosts = Post::select(['id', 'uuid', 'title', 'slug', 'image', 'video_url', 'credit', 'excerpt', 'published_at', 'category_id'])
    ...
    public static function generatePublicId(int $length = 12): string
    {
        ...
        } while (self::where('public_id', $id)->exists()); // ultra-rare loop
