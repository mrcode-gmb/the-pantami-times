  ...
  return (
    <header className="bg-background sticky bg-[#f0a500] border-b-2 border-[#d99200] top-0 z-50 shadow-sm lg:mb-12">
      ...
      <nav className="bg-[#f0a500] border-[#d99200] relative" ref={dropdownRef}>
        ...
        <div className="absolute hidden bg-[#f0a500]s border-b-2 border-[#d99200] lg:block top-full left-0 right-0 bg-muted text-muted-foreground z-50">
              ...
              {navItems.map((item) => {
                ...
                return (
                  <button
                    key={item.label}
                    onClick={() => toggleDropdown(item.label, hasSubcategories, item.href)}
                    className={`px-3 py-3 text-xs news-title-sm border-r border-black text-black hover:bg-[#d99200] transition-colors whitespace-nowrap uppercase tracking-wide flex items-center gap-1 ${isActive ? 'bg-white' : ''
                      }`}
                  >
                    {item.label}
                    {hasSubcategories && (
