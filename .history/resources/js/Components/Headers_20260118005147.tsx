...
export const Header: React.FC<HeaderProps> = ({ categories = [], activeCategory = null }) => {
  ...
  return (
    <header className="bg-background sticky top-0 z-50 shadow-sm lg:mb-12">
              ...
              {navItems.map((item) => {
                ...
                return (
                  <button
                    key={item.label}
                    onClick={() => toggleDropdown(item.label, hasSubcategories, item.href)}
                    className={`px-3 py-3 text-xs border-r border-black text-black hover:bg-[#d99200] transition-colors whitespace-nowrap uppercase tracking-wide flex items-center gap-1 ${isActive ? 'bg-[#b98316]' : ''
                      }`}
                  >
                    {item.label}
                    {hasSubcategories && (
