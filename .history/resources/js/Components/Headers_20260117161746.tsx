              ...
              {navItems.map((item) => {
                ...
                return (
                  <button
                    key={item.label}
                    onClick={() => toggleDropdown(item.label, hasSubcategories, item.href)}
                    className={`px-3 py-2 text-xs font-bold border-r border- text-white hover:bg-[#d99200] transition-colors whitespace-nowrap uppercase tracking-wide rounded flex items-center gap-1 ${isActive ? 'bg-[#d99200]' : ''
                      }`}
                  >
                    {item.label}
                    {hasSubcategories && (
