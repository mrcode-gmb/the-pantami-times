  ...
  return (
    <header className="bg-background sticky bg-[#f0a500] border-b-2 border-[#d99200] top-0 z-50 shadow-sm lg:mb-12">
      ...
      <nav className="bg-[#f0a500] border-[#d99200] relative" ref={dropdownRef}>
        ...
        <div className="containers py-0 border-t-3 border-[#d99200] relative">
              ...
              {navItems.map((item) => {
                ...
                return (
                  <button
                    key={item.label}
                    onClick={() => toggleDropdown(item.label, hasSubcategories, item.href)}
                    className={`px-3 py-3 text-xs news-title-sm border-r border-muted text-black hover:bg-[#d99200] transition-colors whitespace-nowrap uppercase tracking-wide flex items-center gap-1 ${isActive ? 'bg-white' : ''
                      }`}
                  >
                    {item.label}
                    {hasSubcategories && (
