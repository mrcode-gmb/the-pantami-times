  ...
  return (
    <header className="bg-background sticky bg-[#f0a500] border-b-2 border-[#d99200] top-0 z-50 shadow-sm lg:mb-12">
      ...
      <nav className="bg-[#f0a500] border-[#d99200] relative" ref={dropdownRef}>
        ...
        <div className="containers border-0 py-0 relative">
              ...
              {navItems.map((item) => {
                ...
                return (
                  <button
                    key={item.label}
                    onClick={() => toggleDropdown(item.label, hasSubcategories, item.href)}
                    className={`px-3 py-3 text-xs news-title-sm border-r border-black text-black hover:bg-[#d99200] transition-colors whitespace-nowrap uppercase tracking-wide flex items-center gap-1 ${isActive ? 'border-b-4 border-white' : ''
                      }`}
                  >
                    {item.label}
                    {hasSubcategories && (
