...
export const PantamiLogo: React.FC<PantamiLogoProps> = ({ className = '', showText = true }) => {
  return (
    <div className={`flex flex-col items-center justify-center w-full max-w-2xl mx-auto ${className}`}>
      ...
      {/* Text Logo */}
      {showText && (
        <div className="w-full max-w-xl  rounded-sm px-4 py-2 md:px-6 md:py-3">
          ...
          <div className="w-full h-[3px] bg-[#f0a500]">ww</div>
          <hr className="w-full h-[3px] bg-[#f0a500]" />
...
export const PantamiLogoCompact: React.FC<{ className?: string }> = ({ className = '' }) => {
  return (
    <div className={`flex items-center gap-1 md:gap-1 ${className}`}>
      {/* Logo Image */}
      <img 
        src={logo} 
        alt="Pantami Times Logo" 
      ...
      {/* Text */}
      <div className="rounded px-2 py-1 md:px- md:py-1.5">
        <h1 className="font-serif text-sm md:text-xl lg:text-xl font-bold tracking-wide text-foreground transition-colors whitespace-nowrap">
          Pantami Times
        </h1>
        <hr className="w-full h-[3px] my-1 bg-[#f0a500]" />
        <p className="text-[9px] md:text-[13px] lg:text-md text-center text-foreground/70 transition-colors leading-tight">
          Truth, Humanity, & Progress
        </p>
      </div>
    </div>
